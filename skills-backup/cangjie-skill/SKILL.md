---
name: book2skill
description: Distill a BOOK into a coherent set of executable skills. Triggered by: "拆书" / "蒸馏一本书" / "把 XX 书做成 skill" / "turn a book into skills" / "把这本书的方法论拆成 skill" — i.e. explicit reference to a BOOK. NOT for distilling a person's thinking framework (that's huashu-nuwa). When input is a person name (e.g. "蒸馏芒格"), route to huashu-nuwa.
---
...
- ❌ 不做: 书摘 / 读后感 / 作者人设角色扮演 (后者请用 huashu-nuwa)

## 入口歧义路由

收到「蒸馏XX」且不确定 XX 是书还是人名时，**必须反问用户确认**：

```
你是想拆 XX 写的**书**（提取方法论做成可执行 skill），
还是想蒸馏 XX 本人的**思维方式**（做成人物思维框架 skill）？
```

- 用户回答 **书** → cangjie-skill 继续
- 用户回答 **人/思维方式** → 转 huashu-nuwa
